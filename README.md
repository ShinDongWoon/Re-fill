...
**정확한 본문 추출**과 끈질긴 **UI 생존성**을 보장하는 데 집중합니다.
...
이 스크립트의 핵심 난점은 단순히 LLM을 호출하는 일이 아닙니다. 실제 문제는 다음 두 가지입니다.
...
이 프로젝트는 **본문 추출 정확도**와 **페이지 라이프사이클 안정성**을 동일한 최우선순위로 다룹니다. 기본 모델은 별도의 서버 없이 브라우저에서 직접 호출되는 `gemini-3-flash-preview`를 사용합니다.
...
* 우측 rail 도킹은 휴리스틱이므로 사이트 레이아웃 개편에 민감할 수 있습니다.
